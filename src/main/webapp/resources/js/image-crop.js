...
(function() {
  ...
  angular.module('ImageCropper',[])
    ...
    return {
      template: '<div class="ng-image-crop ng-image-crop--{{ shape }}" ng-style="moduleStyles"><section ng-style="sectionStyles" ng-show="step==1"><span class="btn btn-info btn-file botao-selecionar-foto">Selecionar imagem<input type="file" class="image-crop-filepicker" /></span></section><section ng-style="sectionStyles" ng-show="step==2"><canvas class="cropping-canvas" width="{{ canvasWidth }}" height="{{ canvasHeight }}" ng-mousemove="onCanvasMouseMove($event)" ng-mousedown="onCanvasMouseDown($event)" ng-mouseup="onCanvasMouseUp($event)"></canvas><div ng-style="croppingGuideStyles" class="cropping-guide"></div><div class="zoom-handle" ng-mousemove="onHandleMouseMove($event)" ng-mousedown="onHandleMouseDown($event)" ng-mouseup="onHandleMouseUp($event)"><span>&larr; zoom &rarr;</span></div><button class="btn btn-info" ng-click="crop()" data-dismiss="modal">Salvar Imagem</button></section><section ng-style="sectionStyles" class="section-final" ng-show="step==3"><img class="final-cropped-image" ng-src="{{ croppedDataUri }}" /><div style="margin-top: 10px;"><button class="btn btn-warning" ng-show="step==3" data-dismiss="modal">Fechar</button> <button class="btn btn-info" ng-show="step==3" ng-click="result=null; step=1;">Excluir Imagem</button></div></section></div>',
      replace: true,
      restrict: 'AE',
      scope: {
        width: '@',
